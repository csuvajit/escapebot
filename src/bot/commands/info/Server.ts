...
export default class ServerInfoCommand extends Command {
	...
	public async exec(message: Message) {
		...
		const embed = this.client.util.embed()
			...
			.addField('Text Channels', `${message.guild!.channels.cache.filter(ch => ch.isText()).size}`)
			.addField('Voice Channels', `${message.guild!.channels.cache.filter(ch => ch.isVoice()).size}`)
			.addField('AFK', message.guild!.afkChannel ? `<#${message.guild!.afkChannelId!}> after ${message.guild!.afkTimeout / 60} min` : 'None')
